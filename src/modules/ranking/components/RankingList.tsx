...
const ItemContainer = ({ children }: { children: ReactNode }) => {
    return <MotionDiv className="flex justify-between px-5 py-4">{children}</MotionDiv>;
};
        ...
        <>
            <div className="flex">
                <div className="grid place-items-center border border-neutral-100 w-[40px] h-[40px] rounded-full bg-white">
                    <Image src={userImage} width={40} height={40} alt={"next"} />
                </div>
            ...
            <div className="self-center leading-[19px] font-semibold text-secondary-900">{ranking}</div>
        </>
    );
};
