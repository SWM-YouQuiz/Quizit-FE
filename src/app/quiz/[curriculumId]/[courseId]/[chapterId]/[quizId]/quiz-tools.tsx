...
const QuizTools = ({ quizId, likedUserIds, unlikedUserIds }: QuizToolsProps) => {
    ...
    const handleOnClick: MouseEventHandler<HTMLButtonElement> = (event) => {
        ...
        getQuizEvaluate({ id: quizId, isLike: isLike, accessToken }).then((quiz) => {
    ...
    return (
        <div className="flex space-x-1.5">
            <ThumbButton
                handleClick={debouncedClick}
                name="up"
                className={
                    status === "liked" ? "stroke-primary-800 text-primary-800 inner-border-primary-800" : "text-bg-secondary stroke-bg-secondary"
                }
            >
                <ThumbupIcon />
            ...
            <ThumbButton
                ...
                name="down"
                className={
                    status === "unliked" ? "stroke-primary-800 text-primary-800 inner-border-primary-800" : "text-bg-secondary stroke-bg-secondary"
                }
            >
                <ThumbdownIcon />
